...
for i_size in range(n_size):
    ...
    for i_simulation in range(n_simulation):
        ...
        for i_race in range(n_race):
            ...
            n_patient_race = np.count_nonzero(patient_array[:, 0] == i_race)  # number of patients with this race
            marketer_risk_race = 0
            for i_patient in range(base_index_race, (base_index_race + n_patient_race)):
                tuple_patient = tuple(patient_array[i_patient, :])
                marketer_risk_race += 1 / dic_group_size[tuple_patient]
            marketer_risk_race /= n_patient_race
            risk_race[i_race, i_simulation] = marketer_risk_race
            base_index_race += n_patient_race
        ...
        for i_patient in range(n_patient):
            tuple_patient = tuple(patient_array[i_patient, :])
            marketer_risk += 1/dic_group_size[tuple_patient]
        marketer_risk /= n_patient
        #print("Marketer risk: " + str(marketer_risk))
        risk[i_simulation] = marketer_risk
...
with open(Result_folder + "result_R4.txt", 'w') as f:
...
with open(Result_folder + "result_R4_race.txt", 'w') as f:
    for i_race in range(n_race):
