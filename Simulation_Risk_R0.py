...
for i_size in range(n_size):
    ...
    for i_simulation in range(n_simulation):
        ...
        group_size = np.ones(n_group) * n_patient
        for i in range(n_group):
            distance = np.sum(np.absolute(patient_array - unique_rows[i, :]), axis=1)
            group_size_i = np.count_nonzero(distance == 0)
            group_size[i] = group_size_i
            if group_size_i == 1:
                break
        min_group_size = np.min(group_size)
        risk[i_simulation] = 1 / min_group_size
...
with open(Result_folder + "result_R0.txt", 'w') as f:
